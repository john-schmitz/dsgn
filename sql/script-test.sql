...
CREATE UNIQUE INDEX `email_UNIQUE` ON `dsgn`.`cliente` (`email` ASC);
CREATE UNIQUE INDEX `login_UNIQUE` ON `dsgn`.`cliente` (`login` ASC);
CREATE UNIQUE INDEX `cpf_UNIQUE` ON `dsgn`.`cliente` (`cpf` ASC);
...
CREATE INDEX `fk_projeto_designer1_idx` ON `dsgn`.`projeto` (`designer_id` ASC);
CREATE INDEX `fk_projeto_cliente1_idx` ON `dsgn`.`projeto` (`cliente_id` ASC);
...
CREATE INDEX `fk_mensagem_projeto1_idx` ON `dsgn`.`mensagem` (`projeto_id` ASC);
...
CREATE INDEX `fk_designer_has_cliente_cliente1_idx` ON `dsgn`.`proposta` (`cliente_id` ASC);
CREATE INDEX `fk_designer_has_cliente_designer1_idx` ON `dsgn`.`proposta` (`designer_id` ASC);

-- -----------------------------------------------------
-- Table `dsgn`.`imagemPortifolio`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `dsgn`.`imagem_portifolio` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  `designer_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_imagem_portifolio_designer1`
    FOREIGN KEY (`designer_id`)
    REFERENCES `dsgn`.`designer` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE INDEX `fk_imagem_portifolio_designer1_idx` ON `dsgn`.`imagem_portifolio` (`designer_id` ASC);
